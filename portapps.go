...
// AppInfo represents lightdesk.json file structure
type AppInfo struct {
	ID              string `json:"id"`
	...
	URL             string `json:"url"`
	PortappsVersion string `json:"lightdesk_version"`
}
...
func NewWithCfg(id string, name string, appcfg interface{}) (app *App, err error) {
	...
	// Load info
	infoFile := utl.PathJoin("./", "lightdesk.json")
	infoRaw, err := os.ReadFile(infoFile)
	if err != nil {
		app.FatalBox(errors.Wrap(err, "Cannot load lightdesk.json"))
	}
	if err = json.Unmarshal(infoRaw, &app.Info); err != nil {
		app.FatalBox(errors.Wrap(err, "Cannot unmarshal lightdesk.json"))
	}
	...
	log.Info().Msgf("Starting %s %s-%s (lightdesk %s)...", app.Name, app.Info.Version, app.Info.Release, app.Info.PortappsVersion)
	...
	// Load previous
	prevFile := utl.PathJoin(app.RootPath, "-prev.json")
	if utl.Exists(prevFile) {
		prevRaw, err := os.ReadFile(prevFile)
		if err != nil {
			app.FatalBox(errors.Wrap(err, "Cannot load lightdesk-prev"))
		}
		if err = json.Unmarshal(prevRaw, &app.Prev); err != nil {
			log.Error().Err(err).Msgf("Cannot unmarshal lightdesk-prev")
...
func (app *App) Close() {
	...
	// Update previous
	prevFile := utl.PathJoin(app.RootPath, "lightdesk-prev.json")
	...
	if err != nil {
		log.Error().Err(err).Msg("Cannot marshal lightdesk-prev")
	}
	err = os.WriteFile(prevFile, jsonPrev, 0o644)
	if err != nil {
		log.Error().Err(err).Msg("Cannot write lightdesk-prev")
	}
}
