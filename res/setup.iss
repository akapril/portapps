...
begin
  ...
  if (PageId = wpSelectDir) then begin
    ...
    if IsUpgrade then begin
      if FileExists(ExpandConstant('{app}\lightdesk.json')) then begin
        Result := True;
      end
      else begin
        MsgBox(ExpandConstant('The selected dir is not empty or you are performing an upgrade of {#pappName} but required file lightdesk.json cannot be found. Please select a correct folder.'), mbError, MB_OK);
        Result := False;
        exit;
      end;
    end;
